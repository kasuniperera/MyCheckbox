...
const UpdateTask = () => {
    ...
    const { existTitle, existAssignee, existDate, id } = route.params;
    const navigation = useNavigation();

    const [title, setTitle] = React.useState(existTitle);
    const [date, setDate] = React.useState(existDate);
    const [assignees, setAssignees] = React.useState([]);
    const [assigneeLoading, setAssigneeLoading] = React.useState(false);
    const [loading, setLoading] = React.useState(false);
    const [dropdownData, setDropdownData] = React.useState([]);
    const [open, setOpen] = React.useState(false);
    const [dropdownValue, setDropdownVale] = React.useState(existAssignee);

    React.useEffect(() => {
        onGetAssignees();
    }, [])

    //Use to update a task by task id
    const onUpdateTask = () => {
        setLoading(true);
    ...
    //Use to get assignees to dropdown
    ...
    return (
        ...
        <View style={styles.container}>
            ...
            <TextInput
                ...
                value={date} />
            <TouchableOpacity style={styles.button} onPress={onUpdateTask}>
                <PlusIcon />
                <Text style={styles.buttonText}>Update the Task</Text>
            </TouchableOpacity>
            {
                (loading) &&
